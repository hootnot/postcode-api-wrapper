...
class TestUM(unittest.TestCase):
    ...
    def test_PostcodeFail(self):
      ...
      api = API( environment='live', access_key=access_key, access_secret=access_secret)
      pc = ('1077XX', 1)
      self.assertRaises( PostcodeError, lambda: api.get_postcodedata( pc[0], pc[1] ))

    def test_SecretFail(self):
      """ no secret : a request that should fail 
      """
      self.assertRaises( PostcodeError,  lambda: API( environment='live', access_key=access_key))

    def test_KeyFail(self):
      """ no key : a request that should fail 
      """
      self.assertRaises( PostcodeError,  lambda: API( environment='live', access_secret=access_secret))
